...
def summarize_trace(path: str | Path) -> dict[str, object]:
    ...
    fields = ("capture_ms", "preprocess_ms", "encoder_ms", "tail_ms", "postprocess_ms", "end_to_end_ms")
    ...
    duration = sum(float(row.get("frame_interval_ms", 0.0)) for row in rows) / 1000.0
    summary["throughput_fps"] = len(rows) / duration if duration > 0 else None
    return summary
